...
export function addDevice(device: LinkedDevice) {
  const index = loadIndex();
  index.devices = [device, ...index.devices.filter((d) => d.id !== device.id && d.fingerprint !== device.fingerprint)];
  saveIndex(index);
}
...
export function findDevice(fingerprint: string, profilePeerId: string) {
  const index = loadIndex();
  const wantPeer = String(profilePeerId || "").toLowerCase();
  const profile = wantPeer
    ? index.profiles.find((p) => p.peerId && p.peerId.toLowerCase() === wantPeer)
    : null;
  return index.devices.find((d) => d.fingerprint === fingerprint && (!profile || d.profileId === profile.id)) || null;
}
...
export function mergeProfileJson(localJson: string | null, remoteJson: string) {
  ...
  if (local.peerId && remote.peerId && local.peerId !== remote.peerId) {
    throw new Error("Vault profile does not match this identity.");
  }
  const localProfile = asRecord(local.profile);
  const remoteProfile = asRecord(remote.profile);
  const localAt = Number(localProfile.updatedAt) || 0;
  const remoteAt = Number(remoteProfile.updatedAt) || 0;
  const profileUsed = (p: Record<string, unknown>) => Boolean(
    String(p.handle || "").trim() || p.photo || p.dob,
  );
  const profile = remoteAt > localAt
    ? remote.profile
    : localAt > remoteAt
      ? local.profile
      : (profileUsed(remoteProfile) && !profileUsed(localProfile) ? remote.profile : local.profile);
  const chats: Record<string, unknown[]> = { ...(asRecord(local.chats) as Record<string, unknown[]>) };
  for (const [roomId, msgs] of Object.entries(asRecord(remote.chats))) {
    const prev = Array.isArray(chats[roomId]) ? chats[roomId] : [];
    const ids = new Set(prev.map((m) => asRecord(m).id));
    const merged = [...prev];
    if (Array.isArray(msgs)) {
      for (const m of msgs) {
        const rec = asRecord(m);
        if (rec.id && !ids.has(rec.id)) merged.push(m);
      }
    }
    chats[roomId] = merged;
  }
  const friendMap = new Map<string, Record<string, unknown>>();
  for (const f of [...(Array.isArray(local.friends) ? local.friends : []), ...(Array.isArray(remote.friends) ? remote.friends : [])]) {
    const rec = asRecord(f);
    const id = String(rec.peerId || "");
    if (!id) continue;
    const prev = friendMap.get(id);
    friendMap.set(id, prev ? { ...prev, ...rec, handle: rec.handle || prev.handle } : rec);
  }
  const pairState = { ...asRecord(local.pairState), ...asRecord(remote.pairState) };
  const next = {
    ...local,
    ...remote,
    peerId: local.peerId || remote.peerId,
    publicKey: local.publicKey || remote.publicKey,
    privateKey: local.privateKey || remote.privateKey,
    profile,
    friends: [...friendMap.values()],
    chats,
    pairState,
    chatRooms: { ...asRecord(remote.chatRooms), ...asRecord(local.chatRooms) },
    syncRev: Math.max(Number(local.syncRev) || 0, Number(remote.syncRev) || 0, Date.now()),
  };
