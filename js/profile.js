...
const PRIVACY_DEFAULTS_VERSION = 2;
...
export function fieldPrivacy(profile, field, item) {
  ...
  if (field === "lookingFor") {
    ...
    return LOOKING_FOR_PRIVACY_DEFAULTS[id] || "me";
  }
  if (item != null && item !== "") {
...
export function profileComplete(profile) {
  ...
  return Boolean(
    profile.handle?.trim() &&
      isAdult(profile.dob) &&
      profile.gender &&
      (profile.gender !== "self" || profile.genderSelf?.trim()) &&
...
export function publicProfile(profile, peerId) {
  return visibleProfile(profile, peerId, "public");
}

export function shareProfile(profile, peerId, audience = "public") {
  return visibleProfile(profile, peerId, audience);
}

export function visibleProfile(profile, peerId, audience = "public") {
  ...
  const out = {
    ...
    loc: see("loc") ? normalized.loc : null,
