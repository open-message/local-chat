...
export type ShellRPC = {
  bun: {
    requests: {
      ...
      decideLink: {
        params: { secret: string; requestId: string; allow: boolean; profileId: string; importExisting?: boolean };
        response: { ok: boolean };
      };
...
export type HubRPC = {
  bun: {
    requests: {
      ...
      getProfileBlob: { params: { profileId: string }; response: { json: string | null } };
      mergeProfileBlob: { params: { profileId: string; json: string }; response: { json: string } };
    };
    messages: Record<string, never>;
  };
  webview: {
