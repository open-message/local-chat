...
export function trackCustomListItem(listName, itemName) {
  const item = String(itemName || "").trim().slice(0, 40);
  if (!listName || !item) return;
  track("custom_list_item", { list_name: listName, item_name: item });
}
