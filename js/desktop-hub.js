...
function createHttpBridge() {
  ...
  return {
    ...
    getVaultBundle: () => request("getVaultBundle", {}),
    mergeVaultBundle: (params) => request("mergeVaultBundle", params),
  };
}
...
function bindConn(hub, conn) {
  conn.on("data", async (msg) => {
    ...
    if (data.type === "vault-hello") {
      ...
      const bundle = await hub.getVaultBundle({});
      if (bundle.json) conn.send({ type: "vault-state", blob: bundle.json });
      broadcast({ type: "vault-pull" }, conn);
      return;
    }
    if (data.type === "vault-state" && typeof data.blob === "string") {
      ...
      const merged = await hub.mergeVaultBundle({ json: data.blob });
      if (!data.echo) {
        conn.send({ type: "vault-state", blob: merged.json, echo: true });
        broadcast({ type: "vault-state", blob: merged.json, echo: true }, conn);
      }
    }
  });
}
