...
function hubSiteBase() {
  ...
  const raw = configuredOrigin().replace(/\/$/, "");
  try {
...
function hubPageUrl(external = false) {
  ...
  return `${hubSiteBase()}/desktop-hub.html?${params}`;
}
...
function linkedPayload(profileId: string, blob: string) {
...
function authorizeDevice(pending: PendingLink, profileId: string) {
  const blob = vault.readProfile(profileId);
  const hub = vault.getHub();
  if (!blob || !hub) {
  ...
  pending.resolve(linkedPayload(profileId, blob));
  notifyShell();
  return true;
}

function finishLink(pending: PendingLink, { allow, importExisting, profileId }: {
  allow: boolean;
  ...
  if (importExisting) {
    if (!pending.offerBlob) return { ok: false };
    const id = vault.importBrowserProfile(pending.offerBlob);
    return { ok: authorizeDevice(pending, id) };
  }
  const id = profileId || pairingProfileId || vault.getActiveId();
  if (!id) {
    pending.resolve({ allow: false });
    return { ok: false };
  }
...
function targetProfileId() {
  return pairingProfileId || vault.getActiveId();
}
...
const hubRequests: HubRPC["bun"]["requests"] = {
  ...
  reportLink: ({ requestId, fingerprint, label, nonce, devicePublicKey, mode, existingPeerId, existingHandle, existingUsed, offerBlob }) => {
    ...
    if (pending.mode === "import" && pending.offerBlob) {
      try {
        const id = vault.importBrowserProfile(pending.offerBlob);
        vault.addDevice({
          id: vault.newId(),
        ...
        notifyShell();
        return Promise.resolve(linkedPayload(id, vault.readProfile(id) || pending.offerBlob));
      } catch {
        return Promise.resolve({ allow: false });
      }
    }
    const knownPeer = vault.profileIdForPeer(pending.existingPeerId);
    if (knownPeer && (!pairingProfileId || pairingProfileId === knownPeer)) {
      const blob = vault.readProfile(knownPeer);
      if (blob) {
        vault.addDevice({
          id: vault.newId(),
          fingerprint: pending.fingerprint,
        ...
        notifyShell();
        return Promise.resolve(linkedPayload(knownPeer, blob));
      }
    }
    const profileId = targetProfileId();
    if (profileId && (!pending.existingUsed || pending.mode === "replace")) {
      const blob = vault.readProfile(profileId);
      if (blob) {
        vault.addDevice({
          id: vault.newId(),
        ...
        notifyShell();
        return Promise.resolve(linkedPayload(profileId, blob));
      }
    }
  ...
  deviceAllowed: ({ fingerprint, profilePeerId }) => {
    const rec = vault.findDevice(fingerprint, profilePeerId);
    if (!rec) return { ok: false, profileId: "" };
    return { ok: true, profileId: rec.profileId };
  },
  getProfileBlob: ({ profileId }) => ({ json: vault.readProfile(profileId) }),
  mergeProfileBlob: ({ profileId, json }) => {
    const merged = vault.mergeProfileJson(vault.readProfile(profileId), json);
    vault.writeProfile(profileId, merged);
    return { json: merged };
  },
};
...
shellRpc = BrowserView.defineRPC<ShellRPC>({
  handlers: {
    requests: {
      ...
      startPairing: ({ secret }) => {
        requireSecret(secret);
        const activeId = vault.getActiveId();
        if (!activeId) throw new Error("Create a profile first, then Show QR.");
        if (!hubListening) throw new Error("The hub is not online yet.");
        const hub = vault.getHub();
        if (!hub?.hubId) throw new Error("The hub is not ready yet.");
        pairingProfileId = activeId;
      ...
      decideLink: ({ secret, requestId, allow, profileId, importExisting }) => {
        requireSecret(secret);
        const pending = pendingLinks.get(requestId);
        if (!pending) return { ok: false };
        pendingLinks.delete(requestId);
        return finishLink(pending, { allow, importExisting: Boolean(importExisting), profileId });
      },
