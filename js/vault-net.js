...
export class VaultNet {
    ...
    await new Promise((resolve, reject) => {
      ...
      conn.on("open", () => {
        try {
          conn.send({
            ...
            offerBlob: JSON.stringify(exportAllIdentities()),
          });
      ...
      conn.on("data", async (msg) => {
        ...
        if (msg.type === "link-install" && msg.blob) {
          try {
            await installVaultBundle(msg.blob, {
              linkedVault: msg.linkedVault || {
                hubId,
            ...
            finish(err instanceof Error ? err : new Error("Could not install vault identities."));
          }
        }
      });
      conn.on("close", () => {
  ...
  pushState(conn = this.conn) {
    ...
    try {
      conn.send({
        type: "vault-state",
        blob: JSON.stringify(exportAllIdentities()),
  ...
  connectSync(link) {
    ...
    this.peer.on("open", () => {
      ...
      conn.on("data", (msg) => this.onSyncMessage(conn, msg));
  ...
  onSyncMessage(conn, msg) {
    ...
    if (msg.type === "vault-state" && msg.blob) {
      const prev = getState().peerId;
      Promise.resolve(mergeVaultState(msg.blob))
        .then(() => {
          if (getState().peerId !== prev) {
            location.reload();
            return;
          }
          this.onMerged?.();
