import { GROUP_ID_PREFIX, PROTOCOL } from "./config.js";
import { isDevicePeerId, isVaultHubId } from "./device.js";
import { bytesToHex } from "./identity.js";
import { subtle } from "./subtle.js";

const USER_ID_RE = new RegExp(`^${PROTOCOL}u-[0-9a-f]{32}$`, "i");
const GROUP_ID_RE = new RegExp(`^${PROTOCOL}g-[0-9a-f]{32}$`, "i");
...
export function isGroupId(id) {
  return GROUP_ID_RE.test(String(id || ""));
}

export function newGroupId() {
  return `${GROUP_ID_PREFIX}${bytesToHex(crypto.getRandomValues(new Uint8Array(16)))}`;
}
